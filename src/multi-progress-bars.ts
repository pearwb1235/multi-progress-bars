...
export type UpdateOptions = Partial<
  Pick<Task, "message" | "barTransformFn" | "percentage" | "nameTransformFn">
>;
...
export class MultiProgressBars {
  ...
  public constructor(options?: Partial<CtorOptions>) {
    ...
    this.processSimpleBorder(initMessage, border);
  ...
  private processSimpleBorder(
    ...
    if (this.border) {
      this.bottomBorder = clampString(
        this.border.repeat(Math.ceil(this.logger.width / this.border.length)),
        this.logger.width
      );
    }
    ...
    this.bottomBorder !== undefined &&
      this.logger.setBottomBorder(this.bottomBorder);
  }
  ...
  private makeBorder(border: Border) {
    ...
    let { message, left, right } = border;

    // Build the border with only the pattern first
    const base = clampString(
      pattern.repeat(Math.ceil(this.logger.width / stringWidth(pattern))),
      this.logger.width
    );
    ...
    if (right !== undefined) {
      ...
      return clampString(
        base.slice(0, -right - stringWidth(message)) +
          message +
          base.slice(-right),
        this.logger.width
      );
  ...
  public setHeader(options: Border | string | boolean): void {
    if (options !== undefined) {
      if (typeof options === "boolean") {
        ...
        this.logger.setTopBorder(
          clampString(options.split("\n")[0], this.logger.width)
        );
      } else {
  ...
  public setFooter(options: Border | string | boolean): void {
    if (options !== undefined) {
      if (typeof options === "boolean") {
        if (!options) {
  ...
  public addTask(name: string, { index, ...options }: AddOptions): void {
    // Restart promise
    ...
    if (this.tasks[name] !== undefined) {
      ...
      Object.keys(options).forEach(
        (key: keyof Partial<Omit<Task, "index" | "name" | "done">>) =>
    ...
    this.longestNameLength = Math.max(
      this.longestNameLength,
      stringWidth(name)
    );
    ...
    if (options.type === "indefinite" && !this.intervalID) {
      ...
      this.intervalID = setInterval(
        () => this.renderIndefinite(),
        1000 / this.spinnerFPS
      );
    }
  ...
  public isDone(name: string): boolean {
    return this.tasks[stripAnsi(name)].done;
  }

  public removeTask(task: string | number, shift = true): void {
    // Do nothing if task doesn't exist
    if (
      (typeof task === "string" && this.tasks[stripAnsi(task)] === undefined) ||
    ...
    const idxToRemove =
      typeof task === "string" ? this.tasks[stripAnsi(task)].index : task;
    ...
    typeof task === "string"
      ? delete this.tasks[stripAnsi(task)]
      : delete this.tasks[this.getName(task)];

    // Shift up tasks if needed, and also recalculate max task name length for realignment
    this.longestNameLength = Object.entries(this.tasks).reduce(
      (prev, [taskName, { index }]) => {
  ...
  private progressString(task: Task) {
    ...
    const rest =
      scaledInt < this.progressWidth - 1
    ...
    const paddedTaskName = nameTransformFn(
      name.padStart(this.longestNameLength + stringLengthDifference)
    );
  ...
  public incrementTask(
    name: string,
    { percentage = 0.01, ...options }: UpdateOptions = {}
  ): void {
    name = stripAnsi(name);
    if (this.tasks[name] === undefined) {
  ...
  public updateTask(name: string, options: UpdateOptions = {}): void {
    name = stripAnsi(name);
    if (this.tasks[name] === undefined) {
  ...
  public done(
    name: string,
    {
      message = chalk.green("Finished"),
      ...options
    }: Pick<
      UpdateOptions,
      "message" | "barTransformFn" | "nameTransformFn"
    > = {}
  ): void {
    name = stripAnsi(name);
    if (this.tasks[name] === undefined) {
    ...
    if (
      Object.values(this.tasks).reduce((prev, curr) => {
  ...
  public restartTask(
    name: string,
    {
      ...
      UpdateOptions,
      "percentage" | "message" | "barTransformFn" | "nameTransformFn"
    >
  ): void {
    name = stripAnsi(name);
    this.restartPromiseIfNeeded();
  ...
  public getIndex(taskName: string): number | undefined {
  ...
  public getName(index: number): string | undefined {
    return Object.entries(this.tasks).find(
  ...
  private renderIndefinite() {
    ...
    Object.entries(this.tasks).forEach(([, task]) => {
      if (task.type === "indefinite" && task.done === false) {
        const progressString = this.indefiniteString(task, spinner);
        this.logger.upsertProgress({
          index: task.index,
          data: progressString,
        });
