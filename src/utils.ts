 ...
 */
export const CUP = (row: number, column?: number): string =>
 ...
 * @param column (optional) 0-index absolute column
 */
export const CHA = (column: number): string =>
  `${CSI}${numberTo1StringHelper(column)}G`;

/** CUrsor Up
 *
 * @param number (optional) 0-index rows to move up
 */
export const CUU = (number: number): string =>
  `${CSI}${numberTo0StringHelper(number)}A`;

/** CUrsor Down
 *
 * @param number (optional) 0-index rows to move down
 */
export const CUD = (number: number): string =>
  `${CSI}${numberTo0StringHelper(number)}B`;

/** CUrsor Forward
 *
 * @param number (optional) 0-index rows to move right
 */
export const CUF = (number: number): string =>
  `${CSI}${numberTo0StringHelper(number)}C`;

/** CUrsor Back
 *
 * @param number (optional) 0-index rows to move left
 */
export const CUB = (number: number): string =>
  `${CSI}${numberTo0StringHelper(number)}D`;
 ...
 */
export const MoveCursor = (rows: number, columns?: number): string => {
  let command = "";
 ...
 */
export const EL = (mode: EL_MODE = EL_MODE.TO_END): string => {
 ...
 */
export const ED = (mode: ED_MODE = ED_MODE.TO_END): string => {
  return `${CSI}${mode.toString()}J`;
};

// Anyways, probably don't want any styling codes to linger past one line.
export const clampString = (message: string, width: number): string => {
...
export const splitLinesAndClamp = (
  writeString: string,
  maxWidth: number
): string[] => {
